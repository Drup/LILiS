...
module LsEngine (Ls : LSTREAM) = struct
  ...
  let eval_stream (env : Mini_calc.arit_env) l_stream =
    ...
    in Ls.map f_eval_stream l_stream
  ...
  eval_stream env (Ls.of_list rule.rhs)
  ...
  let eval_lsys n lsys = generate_lstream n (Ls.of_list lsys.axiom) (get_transformation lsys)
end
